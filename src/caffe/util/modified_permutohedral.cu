...
static void swapHashTableValues(float* oldValues, float* newValues,
    float* table_values, size_t size) {
...
__global__ static void createMatrix(const int w, const int h,
    const float* positions, int* table_entries, int table_capacity,
    signed short* table_keys, const float* scaleFactor, MatrixEntry* matrix) {
  // 8x8 blocks
  ...
  if (!outOfBounds) {
    ...
    // sort differential to find the permutation between
    // this simplex and the canonical one
    for (int i = 0; i <= pd; ++i) {
      myRank[i] = 0;
      for (int j = 0; j <= pd; ++j) {
    ...
    if (sum > 0) {
      // sum too large, need to bring down the ones
      // with the smallest differential
      for (int i = 0; i <= pd; ++i) {
      ...
      // sum too small, need to bring up the ones with largest differential
      for (int i = 0; i <= pd; ++i) {
        if (myRank[i] < -sum) {
          myGreedy[i] += pd+1;
          myRank[i] += (pd+1) + sum;
...
template<int kd>
__global__ static void cleanHashTable(const int n, int *table_entries,
    int table_capacity, signed short* table_keys) {
...
__global__ static void resetIndex(const int w, const int h,
    MatrixEntry* matrix, int* table_entries) {
  const int x = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__ static void splatCache(const int w, const int h, const int vd,
    const Dtype* values, const MatrixEntry* matrix, float* table_values) {
  const int x = threadIdx.x + blockIdx.x * blockDim.x;
  const int y = threadIdx.y + (blockIdx.y/(pd+1)) * blockDim.y;
  const int threadId = threadIdx.y * blockDim.x + threadIdx.x;
  const int color = blockIdx.y % (pd+1);
  ...
  if (!outOfBounds) {
    ...
    MatrixEntry r = matrix[idx * (pd+1) + color];
    // convert the matrix entry from a pointer into the entries array
    // to a pointer into the keys/values array
  ...
  for (int i = 0; i < BLOCK_SIZE; ++i) {
    if (i < threadId) {
      ...
      if (myOffset == sharedOffsets[i]) {
        // someone else with lower priority cares about this key,
        // accumulate it into mine
        for (int j = 0; j <= vd; j++) {
...
__global__ static void blur(int n, float* newValues,
    const MatrixEntry* matrix, const int* table_entries, const signed short* table_keys,
    const int table_capacity, float* table_values, int color, const int vd) {
  const int idx = (blockIdx.y * gridDim.x + blockIdx.x) * blockDim.x * blockDim.y + threadIdx.x;
  if (idx >= n) {
    return;
  }
...
__global__ static void slice(const int w, const int h, const int vd,
    Dtype* values, const MatrixEntry* matrix,
    float* table_values, bool add) {
  const int x = threadIdx.x + blockIdx.x * blockDim.x;
  const int y = threadIdx.y + blockIdx.y * blockDim.y;
  const int threadId = threadIdx.y * blockDim.x + threadIdx.x;
  const int idx = y * w + x;
...
template<int pd>
void gpu_init(const float* features, HashTable* table, MatrixEntry* matrix,
    const int w, const int h) {
  int num_points = w * h;
  if (num_points <= 0) {
    LOG(FATAL) << "num_points should be greater than 0.";
  }
  // Scan line order
  ...
  // num_dimensions is likely to be low so do that on the CPU
  for (int i = 0; i < pd; ++i) {
...
void gpu_compute(Dtype* out, const Dtype* in, const HashTable &table,
    const MatrixEntry* matrix, int w, int h, int vd, bool reverse, bool add) {
  // Create table_values
  int num_points = w * h;
  if (num_points <= 0) {
    LOG(FATAL) << "num_points should be greater than 0.";
...
void ModifiedPermutohedral::init_gpu(const float* features, int num_dimensions,
    int w, int h) {
...
void ModifiedPermutohedral::compute_gpu(float* out, const float* in,
  ...
  switch (d_) {
    case 2:
      gpu_compute<2, float>(out, in, table, matrix, w_, h_,
          value_size, reverse, add);
      break;
    case 5:
      gpu_compute<5, float>(out, in, table, matrix, w_, h_,
          value_size, reverse, add);
      break;
    default:
      LOG(FATAL) << "num_dimensions should be 2 or 5";
...
void ModifiedPermutohedral::compute_gpu(double* out, const double* in,
    int value_size, bool reverse, bool add) const {
  // Losing time by dynamically allocating memory but more general function
  if (!is_gpu_init_)
    LOG(FATAL) << "Initialize lattice before doing any computing";
  switch (d_) {
    case 2:
      gpu_compute<2, double>(out, in, table, matrix, w_, h_,
          value_size, reverse, add);
      break;
    case 5:
      gpu_compute<5, double>(out, in, table, matrix, w_, h_,
          value_size, reverse, add);
      break;
    default:
      LOG(FATAL) << "num_dimensions should be 2 or 5";
