...
__device__ static int hashTableInsert(unsigned int fh, signed short* key,
    signed short* table_keys, int* table_entries,
    int table_capacity, unsigned int slot) {
  int h = modHash(fh);
  while (1) {
...
__device__ static int hashTableInsert(signed short* key,
  ...
  return hashTableInsert<kd>(myHash, key, table_keys,
      table_entries, table_capacity, slot);
}

template<int kd>
__device__ static int hashTableRetrieve(signed short* key,
    const int* table_entries, const signed short* table_keys,
