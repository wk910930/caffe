...
__global__ void ROIPoolForwardMax(const int nthreads, const Dtype* bottom_data,
...
template <typename Dtype>
__global__ void ROIPoolForwardAve(const int nthreads, const Dtype* bottom_data,
    const Dtype spatial_scale, const int channels, const int height,
    const int width, const int pooled_height, const int pooled_width,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    wstart = min(max(wstart + roi_start_w, 0), width);
    wend = min(max(wend + roi_start_w, 0), width);
    Dtype aveval = Dtype(0);
    int pool_size = (hend - hstart) * (wend - wstart);

    bool is_empty = (hend <= hstart) || (wend <= wstart);

    if (is_empty || !pool_size) {
      top_data[index] = 0;
    } else {
      bottom_data += (roi_batch_ind * channels + c) * height * width;
      for (int h = hstart; h < hend; ++h) {
...
template <typename Dtype>
__global__ void ROIPoolBackwardMax(const int nthreads, const Dtype* top_diff,
...
template <typename Dtype>
__global__ void ROIPoolBackwardAve(const int nthreads, const Dtype* top_diff,
    const int* argmax_data, const int num_rois, const Dtype spatial_scale,
    const int channels, const int height, const int width,
    const int pooled_height, const int pooled_width, Dtype* bottom_diff,
    const Dtype* bottom_rois) {
